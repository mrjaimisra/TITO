class GitPullParser
  ...
  ChangedFile = Struct.new(:additions, :deletions, :number_of_changes)
  def parse(output)
    @additions = 0
    @deletions = 0
    ...
    @changed_files = lines.map do |line|
      ...
      ChangedFile.new(additions: additions, deletions: deletions, number_of_changes: number_of_changes)
    end
